...
import { SLOTS, slotsForSize, SIZE_ORDER } from './config.slots.js';
...
// Autosave is debounced because a finished slot fires several store events in a
// row, and serialising 31 slots on each of them would stutter the board.
function scheduleSave() {
  if (saveTimer) window.clearTimeout(saveTimer);
  saveTimer = window.setTimeout(async () => {
    saveTimer = 0;
    const result = await save.save();
    saveNote = result.ok ? COPY.save.savedNow : COPY.save.failed;
    if (menu) menu.setSavedState(save.hasSave(), save.savedCount());
  }, 900);
}
...
function ensureSession() {
  ...
  session = createSession({
    ...
    onKicked: () => {
      roomState = null;
      peerHeroes = new Map();
      if (board) { board.setRoom(null); board.setSize(worldSize); }
      go('menu');
    },
  });
  return session;
...
function scheduleRestart() {
  if (!session || !roomState || pendingRestart) return;
  const generation = roomState.generation || 0;
  // The host answers first. Everybody else waits a little longer, staggered by
  // their place in the room, so a sleeping host cannot strand the whole team.
  const order = (roomState.players || []).map((p) => p.id).sort().indexOf(session.me);
  const delay = roomState.isHost ? 2000 : 4200 + Math.max(0, order) * 900;
  pendingRestart = window.setTimeout(() => {
    pendingRestart = 0;
    session.restartRun(run ? run.checkpoint : 0, generation, true);
  }, delay);
}

function cancelRestart() {
  if (pendingRestart) window.clearTimeout(pendingRestart);
  pendingRestart = 0;
}
  ...
  if (name === 'editor') {
    ...
    if (runModule && runModule.createRun) {
      ...
      run = runModule.createRun({
        assets: runAssets(),
        seed: runSeed,
        enabled: slotsForSize(currentSize()),
        multiplayer: inRoom,
        ...
        onRestartRequest: inRoom ? () => {
          cancelRestart();
          session.restartRun(run ? run.checkpoint : 0, roomState.generation || 0, false);
        } : null,
      });
      if (inRoom) {
        run.setCheckpoint(roomCheckpoint);
...
// The song is rebuilt from the store wholesale. Feeding it one part at a time is
// what let loops start out of phase and pile up on top of each other.
async function syncSongFromStore() {
  song.stop();
  song.ids.forEach((id) => song.clearTrack(id));
  for (let i = 0; i < MUSIC.tracks.length; i += 1) {
    const asset = store.getAsset(MUSIC.tracks[i]);
    if (asset && asset.sound && asset.sound.blob) await song.setTrack(MUSIC.tracks[i], asset.sound.blob);
  }
}

// Whatever just landed in the store goes out to the room, slot by slot, under the
// same first-come rule as drawing by hand.
async function publishToRoom(slotIds) {
  if (!session || !roomState || !roomState.code) return 0;
  let sent = 0;
  for (let i = 0; i < slotIds.length; i += 1) {
    ...
    const asset = store.getAsset(slotId);
    if (slotId === 'hero') {
      if (asset.bitmap) await session.publishAsset('hero', asset.bitmap, asset.rig, asset.sound);
      continue;
    }
    if (!(await session.claimSlot(slotId))) continue;
    if (slot.music) await session.publishMusic(slotId, asset.sound);
    else await session.publishAsset(slotId, asset.bitmap, asset.rig, asset.sound);
    sent += 1;
  }
  return sent;
}

// Carrying a saved world into a room fills only what nobody has claimed, so two
// people arriving with their own sets cannot overwrite each other.
async function bringSetIntoRoom() {
  const payload = save.readSaved();
  if (!payload || !session) return 0;
  const claimed = Object.keys(roomState && roomState.slots ? roomState.slots : {});
  const applied = await save.apply(payload, { onlyEmpty: true, skip: claimed });
  await publishToRoom(applied);
  await syncSongFromStore();
...
function readSize() {
  try {
    const value = window.localStorage.getItem('dr.size');
    return SIZE_ORDER.indexOf(value) !== -1 ? value : 'medium';
  } catch (error) {
    return 'medium';
  }
}
...
function currentSize() {
  return session && roomState && roomState.code ? (roomState.size || 'medium') : worldSize;
}
...
async function boot() {
  ...
  rooms = createRooms({
    ...
    onHost: async (nick, bring) => {
      const s = ensureSession();
      newSet();
      await s.host(nick, worldSize);
      if (bring) await bringSetIntoRoom();
      await go('board');
    },
    onJoin: async (code, nick, bring) => {
      const s = ensureSession();
      newSet();
  ...
  menu.setSavedState(save.hasSave(), save.savedCount());
  ...
  board = createBoard({
    ...
    onSize: (next) => {
      if (session && roomState && roomState.code) {
        session.setSize(next);
        return;
      }
      worldSize = next;
      try { window.localStorage.setItem('dr.size', next); } catch (error) { /* not remembered, still applied */ }
      board.setSize(next);
    },
    onExport: () => save.exportFile('doodlerun-world'),
    onImport: async () => {
      const inRoom = !!(session && roomState && roomState.code);
      const claimed = inRoom ? Object.keys(roomState.slots || {}).filter((id) => (roomState.slots[id] || {}).owner !== session.me) : [];
      const applied = await save.importFile(inRoom ? { onlyEmpty: false, skip: claimed } : null);
      if (!applied) return null;
      if (inRoom) await publishToRoom(applied);
      await syncSongFromStore();
      board.refresh(null);
      return applied.length;
    },
