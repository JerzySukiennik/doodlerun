...
export function createTrack(seed, enabled) {
  // `enabled` is the list of slots this world contains. Anything outside it is
  // simply never generated, so every client with the same size sees the same track.
  const has = (slotId) => !enabled || enabled.indexOf(slotId) !== -1;
  const tile = RUN.tile;
  ...
  function addHazard(kind, fromTile, tiles, y, extra) {
    if (kind !== 'fire') lastDangerTile = Math.max(lastDangerTile, fromTile + tiles + (extra && extra.range ? extra.range / tile : 0));
  ...
  function addSpikes(fromTile, tiles) {
    lastDangerTile = Math.max(lastDangerTile, fromTile + tiles);
    hazards.push({
      id: nextId(),
      kind: 'spikes',
      x: fromTile * tile,
  ...
  function emitSection(metres) {
    ...
    const menu = [
      ...
      { id: 'lucky', weight: 11, ok: metres > 25 && has('luckyBlock') },
      { id: 'saw', weight: 13, ok: runTiles >= 6 && metres > 45 && has('saw') },
      { id: 'rock', weight: 9, ok: metres > 70 && has('rock') },
      { id: 'trampoline', weight: 9, ok: runTiles >= 5 && metres > 55 && has('trampoline') },
      { id: 'conveyor', weight: 9, ok: runTiles >= 6 && metres > 90 && has('conveyor') },
      { id: 'mover', weight: 12, ok: metres > 100 && has('mover') },
      { id: 'nothing', weight: 6, ok: true },
    ].filter((entry) => entry.ok)
      .filter((entry) => ['spikes', 'saw', 'rock'].indexOf(entry.id) === -1 || start + 2 - lastDangerTile >= RUN.dangerSpacingTiles - 2);
    ...
    if (feature === 'spikes') {
      ...
      const at = start + 2;
      const span = Math.min(runTiles - 4, 2 + Math.floor(rand() * 2));
      if (span > 0) addHazard('saw', at, 1, groundTop, { range: span * tile, speed: 90 + rand() * 70, phase: rand() * 6 });
    ...
    // One danger at a time: a creature never shares a stretch with a hazard, and
    // it keeps its distance from the last one so there is room to land and react.
    const dangerous = feature === 'spikes' || feature === 'saw' || feature === 'rock';
    const creatureAt = start + Math.max(2, Math.floor(runTiles * 0.6));
    const clear = creatureAt - lastDangerTile >= RUN.dangerSpacingTiles;
    if (!dangerous && clear && metres > 60 && rand() < 0.30 + d * 0.35) {
    ...
    if (rand() < 0.30 && metres > 90) {
      ...
      const weights = [0.34, has('magnet') ? 0.28 : 0, has('clock') ? 0.24 : 0, has('heart') ? 0.14 : 0];
      let pick = powers[0];
      let roll2 = rand();
    ...
    const wantsGap = rand() < 0.26 + d * 0.20 && metres > 60;
    if (wantsGap) {
      const maxGap = Math.max(1, Math.min(3, Math.floor(reachTiles * 0.45)));
      const gap = 1 + Math.floor(rand() * maxGap);
      if (has('fire')) addHazard('fire', cursorTile, gap, 2.2 * tile, {});
      if (rand() < 0.35 && gap >= 2) {
        addPlatform(cursorTile + Math.floor(gap / 2), 1, 2.9);
        if (rand() < 0.6) addPickup('coin', cursorTile + Math.floor(gap / 2), -3.9 * tile);
      }
      cursorTile += gap;
    }
  }
