...
export function createRun(options) {
  ...
  function reset() {
    track = createTrack(seed, enabled);
  ...
  function openLucky(block) {
    ...
    if (Math.random() < 0.62) {
      ...
      const powers = ['shield', 'magnet', 'clock', 'heart'].filter(has);
  ...
  function die(cause, remote) {
    ...
    overWait.hidden = !multiplayer;
    overWait.textContent = canRestart ? COPY.run.restartingSoon : COPY.run.waitingHost;
  ...
  function stepCreature(entity, dt) {
    ...
    // A creature sleeps until it is nearly on screen. If it set off the moment it
    // was generated it would wander back over the hazard in the section before it,
    // and the two together are a wall nobody can clear.
    if (!entity.awake) {
      if (entity.x - hero.x > RUN.wakeDistance) return;
      entity.awake = true;
    }
  ...
  function stepChaser(dt, speed, nominal) {
    ...
    if (!chaser && has('chaser') && slowFor > RUN.chaserWakeMs) {
  ...
  function step(dt) {
    ...
    const canJump = hero.grounded || coyote > 0 || hero.jumps < RUN.maxJumps;
    if (jumpBuffer > 0 && canJump) {
      if (hero.grounded || coyote > 0) {
        hero.vy = -RUN.jumpV;
        hero.jumps = 1;
      } else {
        // Each jump in the air is a touch weaker than the one before it.
        hero.vy = -(hero.jumps >= 2 ? RUN.thirdJumpV : RUN.doubleJumpV);
        hero.jumps += 1;
      }
      hero.grounded = false;
      coyote = 0;
    ...
    while (has('gate') && metres >= nextGate) {
  ...
  return {
    ...
    remoteDeath(info) {
      ...
      // No grace period here: the run is over for everyone the moment it is
      // over for one, and it has to look that way on every screen at once.
      over.hidden = false;
    ...
    get state() {
      return {
        metres, coins, score: score(), dead, shield, fps: fpsValue, mesh: cols,
        y: hero ? hero.y : 0, jumps: hero ? hero.jumps : 0, grounded: hero ? hero.grounded : false,
      };
    },
  };
}
