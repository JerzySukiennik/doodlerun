...
import { NET, MUSIC } from '../config.js';
import { SLOT_BY_ID, MUSIC_SLOTS, slotsForSize } from '../config.slots.js';
...
export function createSession(options) {
  ...
  const onMusic = (options && options.onMusic) || (() => {});
  ...
  async function watchRoom() {
    ...
    watchers.push(driver.watch(room('meta'), (value) => {
      ...
      if (value && before && wasRunning && value.phase === 'run' && value.generation !== before.generation) {
        onRun({ kind: 'restart', seed: value.seed, generation: value.generation, checkpoint: value.checkpoint || 0 });
      }
      if (value && value.phase === 'run' && (!before || before.phase !== 'run')) {
        onRun({ kind: 'start', seed: value.seed, generation: value.generation || 0, checkpoint: value.checkpoint || 0 });
      }
    ...
    watchers.push(driver.watchChildren(room('music'), {
      added: (trackId, value) => deliverMusic(trackId, value),
      changed: (trackId, value) => deliverMusic(trackId, value),
      removed: (trackId) => onMusic({ trackId, sound: null, by: null }),
    }));

    watchers.push(driver.watch(room('death'), (value) => {
      if (!value || value.by === me) return;
      // A death left over from an earlier attempt must not end this one.
      if ((value.generation || 0) !== ((live.meta || {}).generation || 0)) return;
  ...
  function deliverMusic(trackId, value) {
    if (!value || value.by === me) return;
    onMusic({ trackId, sound: decodeSound(value), by: value.by });
  }
  ...
  async function enter(nextCode, nick, asHost, hostSize) {
    code = nextCode;
    saveNick(nick);
  ...
  return {
    ...
    async listRooms(handler) {
      ...
      return driver.watch('lobbies', (value) => {
        ...
        Object.keys(value || {}).forEach((key) => {
          ...
          if ((row.updated || 0) < cutoff) {
            // Whoever opens the list clears out rooms nobody came back to.
            if ((row.updated || 0) < nowMs() - NET.lobbyTtlMs * 4) {
              driver.remove(`lobbies/${key}`);
              driver.remove(`rooms/${key}`);
            }
            return;
          }
    ...
    async host(nick, size) {
      ...
      await enter(next, nick, true, size);
      return next;
    },
    ...
    async leave() {
      ...
      try {
        ...
        await driver.update(room(''), updates);
        // Ask the database who is left rather than trusting a snapshot that may
        // predate somebody else's exit, or the room is never cleaned up.
        const remaining = await driver.get(room('players'));
        const rest = Object.keys(remaining || {}).filter((id) => id !== me);
    ...
    async publishMusic(trackId, sound) {
      if (!code) return;
      if (!sound) {
        await driver.remove(room(`music/${trackId}`));
        await driver.remove(room(`slots/${trackId}`));
        return;
      }
      const packed = await encodeSound(sound, MUSIC.maxBytes);
      if (!packed) return;
      await driver.set(room(`music/${trackId}`), Object.assign({ by: me, at: nowMs() }, packed));
      await driver.update(room(`slots/${trackId}`), { owner: me, done: true, at: nowMs() });
    },

    // Everybody in the room owes the song one part; that is the gate, not the
    ...
    playersWithoutPart() {
      ...
      Object.keys(slots).forEach((slotId) => {
        const slot = slots[slotId];
        if (MUSIC_SLOTS.indexOf(slotId) !== -1 && slot && slot.done && slot.owner) recorded[slot.owner] = true;
      });
      return Object.keys(players).filter((id) => !recorded[id]);
    },

    async setSize(size) {
      if (!code || (live.meta || {}).host !== me) return;
      await driver.update(room('meta'), { size });
    },
    ...
    missingSlots() {
      const slots = live.slots || {};
      return slotsForSize((live.meta || {}).size || 'medium').filter((slotId) => !(slots[slotId] && slots[slotId].done));
    },
    ...
    // The restart is a transaction keyed on the generation it answers, so two
    // clients reacting to the same death can never send the room back twice.
    async restartRun(checkpoint, expectedGeneration, anyone) {
      if (!code) return false;
      if (!anyone && (live.meta || {}).host !== me) return false;
      const expected = Number.isFinite(expectedGeneration) ? expectedGeneration : ((live.meta || {}).generation || 0);
      const res = await driver.transaction(room('meta'), (current) => {
        if (!current || (current.generation || 0) !== expected) return undefined;
        return Object.assign({}, current, {
          generation: expected + 1,
          checkpoint: Math.max(0, Math.floor(Number(checkpoint) || 0)),
        });
      });
