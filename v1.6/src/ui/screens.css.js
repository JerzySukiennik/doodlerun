...
.dr-rooms-bring{display:flex;align-items:center;gap:10px;
font:700 var(--dr-t-label) var(--dr-face-mono);letter-spacing:var(--dr-tr-label);text-transform:uppercase}
.dr-rooms-bring input{width:20px;height:20px;accent-color:var(--dr-ink)}
.dr-file-acts{display:flex;align-items:center;gap:6px;margin-left:auto;flex:0 0 auto}
.dr-file-acts .dr-btn{min-height:36px;padding:0 12px;font-size:10px}
.dr-file-note{font:700 var(--dr-t-micro) var(--dr-face-mono);letter-spacing:var(--dr-tr-micro);
text-transform:uppercase;color:var(--dr-mute)}
...
.dr-studio{background:var(--dr-ground);display:flex;flex-direction:column;
padding:28px clamp(28px,5vw,80px);gap:var(--dr-gap-l);overflow-y:auto}
.dr-studio-head{display:grid;grid-template-columns:auto 1fr auto;
grid-template-areas:"back title done" "back tip done";
gap:6px var(--dr-gap-l);align-items:center}
.dr-studio-head > .dr-btn-ghost{grid-area:back;align-self:start}
.dr-studio-title{grid-area:title;margin:0;font:900 var(--dr-t-h1) var(--dr-face-display);
letter-spacing:var(--dr-tr-h1);text-transform:uppercase}
.dr-studio-tip{grid-area:tip;margin:0;max-width:64ch;
font:500 var(--dr-t-body) var(--dr-face-display);color:var(--dr-mute)}
.dr-studio-done{grid-area:done}
.dr-studio-body{display:flex;flex-direction:column;gap:var(--dr-gap-m);align-items:stretch}
.dr-studio-stage{--dr-off:var(--dr-off3);padding:var(--dr-pad);display:flex;flex-direction:column;gap:var(--dr-gap-m)}
.dr-studio-state{font:900 var(--dr-t-h2) var(--dr-face-display);letter-spacing:var(--dr-tr-h2);
text-transform:uppercase}
.dr-studio[data-phase="count"] .dr-studio-state{color:var(--dr-sun)}
.dr-studio[data-phase="record"] .dr-studio-state{color:var(--dr-alert)}
.dr-studio-beats{display:flex;gap:8px;flex-wrap:wrap}
.dr-studio-beat{width:26px;height:26px;border:var(--dr-b2) solid var(--dr-ink);background:var(--dr-paper);
display:block}
.dr-studio-beat.is-countin{border-style:dashed}
.dr-studio-beat.is-on{background:var(--dr-sun)}
.dr-studio-beat.is-countin.is-on{background:var(--dr-ink)}
.dr-studio-wave{width:100%;height:150px;border:var(--dr-b3) solid var(--dr-ink);background:var(--dr-paper);
display:block}
.dr-studio-meta{margin:0;font:700 var(--dr-t-label) var(--dr-face-mono);letter-spacing:var(--dr-tr-label);
text-transform:uppercase;color:var(--dr-mute)}
.dr-studio-acts{display:flex;gap:var(--dr-gap-s);flex-wrap:wrap}
.dr-studio-record.is-live{--dr-fill:var(--dr-alert);color:var(--dr-alert-ink);animation:dr-blink .9s steps(1) infinite}
.dr-studio-toggle{display:flex;align-items:center;gap:10px;
font:700 var(--dr-t-label) var(--dr-face-mono);letter-spacing:var(--dr-tr-label);text-transform:uppercase}
.dr-studio-toggle input{width:20px;height:20px;accent-color:var(--dr-ink)}
.dr-studio-hint{margin:0;font:500 var(--dr-t-body) var(--dr-face-display);color:var(--dr-mute)}
.dr-studio-mic{margin:0;padding:9px 12px;background:var(--dr-alert);color:var(--dr-alert-ink);
font:700 var(--dr-t-label) var(--dr-face-mono);letter-spacing:var(--dr-tr-label);text-transform:uppercase}
.dr-slot-music .dr-slot-thumb{display:none}
.dr-slot-music{min-height:132px}

.dr-sizebar{display:flex;align-items:center;gap:var(--dr-gap-m);flex-wrap:wrap;margin:0 0 var(--dr-gap-m)}
.dr-sizebar .dr-seg button[disabled]{cursor:default;opacity:.55}
.dr-size-note{font:500 var(--dr-t-body) var(--dr-face-display);color:var(--dr-mute)}
