...
import { CATEGORIES, SLOTS, slotsByCategory, SIZE_ORDER, inSize } from '../config.slots.js';
...
export function createBoard(options) {
  ...
  const roomBar = el('div', 'dr-roombar', root);
  roomBar.hidden = true;
  const roomCode = el('strong', 'dr-roombar-code', roomBar, '');
  const roomPlayers = el('div', 'dr-roombar-players', roomBar);
  const roomNote = el('span', 'dr-roombar-note', roomBar, '');
  const roomLeave = button(COPY.room.leave, 'dr-btn-ghost dr-roombar-leave', roomBar, () => opts.onLeaveRoom && opts.onLeaveRoom());
  ...
  const sizeBar = el('div', 'dr-sizebar', root);
  el('span', 'dr-field-label', sizeBar, COPY.size.label);
  const sizeSeg = el('div', 'dr-seg', sizeBar);
  const sizeButtons = {};
  SIZE_ORDER.forEach((id) => {
    const node = el('button', null, sizeSeg, COPY.size[id]);
    node.type = 'button';
    node.addEventListener('click', () => {
      if (room && !room.isHost) return;
      if (opts.onSize) opts.onSize(id);
    });
    sizeButtons[id] = node;
  });
  const sizeNote = el('span', 'dr-size-note', sizeBar, '');
  let size = 'medium';
  ...
  CATEGORIES.filter((cat) => cat.id !== 'hero').forEach((cat) => {
    ...
    const countNode = el('span', 'dr-group-count', groupHead, '');
    groups.push({ node: group, slots, countNode, id: cat.id });
  ...
  const countDrawn = el('span', null, counts, copy.drawn(0, SLOTS.length));
  const countRigged = el('span', null, counts, copy.rigged(0));
  const countSound = el('span', null, counts, copy.sound(0));
  const fileActs = el('div', 'dr-file-acts', bar);
  const exportBtn = button(COPY.save.export, 'dr-btn-ghost', fileActs, async () => {
    const n = await opts.onExport();
    fileNote.textContent = COPY.save.saved(n);
  });
  const importBtn = button(COPY.save.import, 'dr-btn-ghost', fileActs, async () => {
    const n = await opts.onImport();
    fileNote.textContent = n === null ? COPY.save.importFailed : COPY.save.imported(n);
  });
  const fileNote = el('span', 'dr-file-note', fileActs, '');
  ...
  function stateWords(slot, asset) {
    if (slot.music) return asset.sound && asset.sound.url ? copy.stateRecorded : copy.stateNoTake;
  ...
  function updateCard(slotId) {
    ...
    record.card.classList.toggle('dr-slot-music', !!slot.music);
    if (slot.music) {
      const recorded = !!(asset.sound && asset.sound.url);
      record.card.classList.toggle('is-done', recorded);
      record.card.classList.toggle('is-ready', recorded);
      record.card.classList.toggle('is-empty', !recorded);
      record.card.classList.remove('is-progress', 'is-working');
    } else {
  ...
  // The size decides what is on the board at all: a slot outside it is not
  // waiting to be drawn, it is simply not part of this world.
  function applySize() {
    SIZE_ORDER.forEach((id) => sizeButtons[id].classList.toggle('is-on', id === size));
    const locked = !!room && !room.isHost;
    SIZE_ORDER.forEach((id) => { sizeButtons[id].disabled = locked; });
    sizeNote.textContent = locked ? COPY.size.hostOnly : COPY.size.note[size];
    cards.forEach((record, slotId) => {
      record.card.style.display = inSize(size, slotId) ? '' : 'none';
    });
    groups.forEach((entry) => {
      const shown = entry.slots.filter((slot) => inSize(size, slot.id)).length;
      entry.node.style.display = shown > 0 ? '' : 'none';
      entry.countNode.textContent = copy.groupCount(entry.id, shown);
    });
    SLOTS.forEach((slot, i) => { pips[i].style.display = inSize(size, slot.id) ? '' : 'none'; });
  }
  ...
  function updateBar() {
    ...
    assets.forEach((asset, i) => {
      if (!inSize(size, asset.slotId)) return;
      sharedTotal += 1;
      const pip = pips[i];
    ...
    // Solo you may run a half-drawn world; in a room the set has to be finished,
    // because it is everyone's world and everyone's run.
    const openSlots = (room.missing || []).length;
    const heroless = (room.heroless || []).length;
    startBtn.textContent = COPY.room.startTogether;
    startBtn.disabled = !room.isHost || openSlots > 0 || heroless > 0;
    startNote.textContent = openSlots > 0 ? COPY.room.waitingSlots(openSlots)
      : heroless > 0 ? COPY.room.waitingHeroes(heroless)
      : room.isHost ? COPY.room.readyToRun
      : COPY.room.hostOnly;
  }

  // In a room a slot belongs to whoever clicked it first; everyone else sees it
  // locked with the name of the person drawing.
  function applyLock(record) {
    const card = record.card;
    let lockNote = card.querySelector('.dr-slot-lock');
    const slotId = record.slot.id;
    const claim = room && record.slot.shared ? (room.slots || {})[slotId] : null;
    const mine = claim && claim.owner === room.me;
    const foreign = !!claim && !mine;
    card.classList.toggle('is-locked', foreign);
    card.disabled = foreign;
    if (!foreign) {
      if (lockNote) lockNote.remove();
      return;
    }
    if (!lockNote) lockNote = el('span', 'dr-slot-lock', card, '');
    const owner = (room.players || []).find((p) => p.id === claim.owner);
    lockNote.textContent = claim.done ? COPY.room.taken : COPY.room.drawing(owner ? owner.nick : '…');
  }

  function renderRoom() {
    roomBar.hidden = !room;
    if (!room) {
      cards.forEach((record) => applyLock(record));
      updateBar();
      return;
    }
    roomCode.textContent = `${COPY.room.label} ${room.code}`;
    roomPlayers.textContent = '';
    (room.players || []).forEach((player) => {
      const chip = el('div', 'dr-roombar-player', roomPlayers);
      if (player.id === room.me) chip.classList.add('is-me');
      if (player.id === room.host) chip.classList.add('is-host');
      el('span', 'dr-roombar-nick', chip, player.nick || '…');
      const tag = player.id === room.me ? COPY.room.you : player.id === room.host ? COPY.room.host : '';
      if (tag) el('span', 'dr-roombar-tag', chip, tag);
      if (room.isHost && player.id !== room.me) {
        const kick = el('button', 'dr-roombar-kick', chip, COPY.room.kick);
        kick.type = 'button';
        kick.addEventListener('click', () => opts.onKick && opts.onKick(player.id));
      }
    });
    cards.forEach((record) => applyLock(record));
    updateBar();
  ...
  function setRoom(next) {
    room = next || null;
    if (room && room.size && room.size !== size) size = room.size;
    applySize();
    renderRoom();
  }

  function setSize(next) {
    if (SIZE_ORDER.indexOf(next) === -1) return;
    size = next;
    applySize();
    updateBar();
  }

  return { root, show, hide, dispose, refresh, setRoom, setSize, showSummary: openSummary, closeSummary };
}
