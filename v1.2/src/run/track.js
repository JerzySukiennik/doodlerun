...
export function createTrack(seed) {
  ...
  function emitSection(metres) {
    ...
    if (metres > 30 && roll < 0.26 + d * 0.12 && runTiles >= 6) {
      ...
      // A platform has to clear a running head: its underside sits at
      // (tile - lift * tile), so anything under lift 3 is a wall at head height.
      ...
      // A platform is the only ceiling this world has, so that is where the
      // ceiling crawler belongs.
