...
function handleRunEvent(event) {
  ...
  if (event.kind === 'start' || event.kind === 'restart') {
    ...
    if (current === 'run' && run) run.restart();
    else go('run');
    return;
  }
  if (event.kind === 'stop') {
  ...
  if (name === 'editor') {
    ...
    if (runModule && runModule.createRun) {
      ...
      run = runModule.createRun({
        ...
        onDeath: (cause, metres) => {
          session.reportDeath(cause, metres);
          scheduleRestart();
        },
        onTick: (x, y, metres, stateName) => session.publishLive(x, y, metres, stateName),
        onRestartRequest: () => session.restartRun(),
      });
      if (inRoom) {
        applyPeerHeroes();
        watchPeers();
