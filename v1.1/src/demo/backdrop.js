...
export function createBackdrop(options) {
  ...
  function drawLayer(slotId, speed, height, bottom) {
    const entry = statics.get(slotId);
    if (!entry) return;
    const tileW = entry.width * (height / entry.height);
    if (!(tileW > 1)) return;
    const offset = wrap(time * speed, tileW);
    const top = bottom - height;
    for (let x = -offset; x < view.w + tileW; x += tileW) {
      ctx.drawImage(entry.texture, x, top, tileW, height);
    }
  }
  ...
  function drawSprite(slotId, x, top, phase, flip) {
    ...
    ctx.save();
    if (flip) {
      ctx.translate(x, top + sprite.height * view.scale);
  ...
  function drawCreatures() {
    ...
    for (let i = 0; i < BACKDROP.creatureCount; i += 1) {
      ...
      if (slotId === 'crawler') {
        drawSprite(slotId, x, view.bandTop - (96 - BASELINE.crawler) * view.scale, PHASE[i], true);
        continue;
      }
      if (slotId === 'flyer') {
  ...
  function draw() {
    ...
    ctx.fillStyle = SHELL.color.ground;
    ctx.fillRect(0, 0, view.w, view.h);
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, view.bandTop, view.w, view.bandBottom - view.bandTop);
    ctx.clip();
    ctx.fillStyle = SHELL.color.paper;
    ctx.fillRect(0, view.bandTop, view.w, view.bandBottom - view.bandTop);
    drawStatic('sky', 0, view.bandTop, view.w, view.groundY - view.bandTop);
    drawLayer('far', BACKDROP.farSpeed, view.air * 0.72, view.groundY);
    drawLayer('near', BACKDROP.nearSpeed, view.air * 0.5, view.groundY + 3);
    drawGround();
    drawProps();
    drawLoot();
    drawCreatures();
