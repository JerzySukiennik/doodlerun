...
async function bringSetIntoRoom() {
  ...
  for (let i = 0; i < applied.length; i += 1) {
    const slotId = applied[i];
    const slot = SLOTS.find((entry) => entry.id === slotId);
    if (!slot || !slot.shared) continue;
  ...
  if (hero && hero.bitmap) await session.publishAsset('hero', hero.bitmap, hero.rig, hero.sound);
...
async function boot() {
  ...
  board = createBoard({
    ...
    onImport: async () => {
      const applied = await save.importFile();
      if (!applied) return null;
      board.refresh(null);
      applied.forEach((slotId) => {
        const asset = store.getAsset(slotId);
        if (asset && asset.sound && asset.sound.blob && MUSIC.tracks.indexOf(slotId) !== -1) {
          song.setTrack(slotId, asset.sound.blob);
        }
      });
      return applied.length;
    },
