...
import { SLOTS, SLOT_BY_ID, CORE, MUSIC_SLOTS } from '../config.slots.js';
...
// Only the core gates a run. The rest can be filled in between attempts.
const SHARED = CORE.slice();
...
export function createSession(options) {
  ...
  return {
    ...
    async restartRun(checkpoint) {
      if (!code || (live.meta || {}).host !== me) return;
      await driver.remove(room('death'));
      await driver.remove(room('live'));
      await driver.update(room('meta'), {
        generation: ((live.meta || {}).generation || 0) + 1,
