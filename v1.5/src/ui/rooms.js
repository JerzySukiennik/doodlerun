...
export function createRooms(options) {
  ...
  const bringWrap = el('label', 'dr-rooms-bring', side);
  const bringBox = el('input', null, bringWrap);
  bringBox.type = 'checkbox';
  el('span', null, bringWrap, COPY.save.bring);
  const bringNote = el('p', 'dr-field-note', side, COPY.save.bringNote);
  bringWrap.hidden = !opts.hasSave;
  bringNote.hidden = !opts.hasSave;
  ...
  async function doHost() {
    ...
    setBusy(true);
    try {
      await opts.onHost(nick(), bringBox.checked);
    } catch (err) {
  ...
  async function doJoin(code) {
    ...
    setBusy(true);
    try {
      await opts.onJoin(code, nick(), bringBox.checked);
    } catch (err) {
  ...
  return {
    ...
    setHasSave(has) {
      bringWrap.hidden = !has;
      bringNote.hidden = !has;
    },
  };
}
