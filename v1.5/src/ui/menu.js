...
export function createMenu(options) {
  ...
  function activate(id) {
    ...
    else if (id === 'together' && opts.onTogether) opts.onTogether();
  }
  ...
  function refresh() {
    ...
    if (cont) {
      // Continue means the world on disk, not whatever is still in memory.
      const off = drawn === 0 && !hasSaved;
      cont.item.disabled = off;
      cont.note.textContent = off ? cont.entry.noteOff
        : hasSaved ? COPY.save.saved(savedCount)
        : cont.entry.note;
    }
  ...
  function setSavedState(has, count) {
    hasSaved = !!has;
    savedCount = count || 0;
    refresh();
  }
  ...
  return { root, show, hide, dispose, refresh, setDemoReady, setSavedState };
}
